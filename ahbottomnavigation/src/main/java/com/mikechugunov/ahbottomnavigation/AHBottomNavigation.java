...
import com.mikechugunov.ahbottomnavigation.notification.AHNotification;
import com.mikechugunov.ahbottomnavigation.notification.AHNotificationHelper;
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (current) {
				...
				icon.setImageDrawable(item.getSelectedDrawable(context));
			...
			if (forceTint) {
				...
				if (current) {
					icon.setImageDrawable(items.get(i).getSelectedDrawable(context));
				}
			}
